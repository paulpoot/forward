...
const getEntryCollection = async (contentType, query = {}) => {
    ...
    const url = `${cdnUrl}/spaces/${process.env.CONTENTFUL_SPACE_ID}/environments/master/entries?${baseParameters}`;
...
const getEntry = async (id, contentType) => {
    ...
    const url = `${cdnUrl}/spaces/${process.env.CONTENTFUL_SPACE_ID}/environments/master/entries/${id}?${baseParameters}`;
